...
const Authpage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // you might want to enforce the checks before signup:
        // if (state === 'signup' && (!hasUppercase || !minLength || !isStrong)) { ... }
    ...
    // strong: at least one lowercase, one uppercase, one digit and one special char, and length >= 5
    const isStrong = /(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\w\s]).{5,}/.test(password);

    const getItem = (ok, text) => (
        <li className="flex items-center gap-2 text-xs">
            {ok ? <Check size={16} className="text-[#06b6d4]" /> : <X size={16} className="text-gray-500" />}
            <span className={ok ? "text-[#06b6d4]" : "text-gray-400"}>{text}</span>
        </li>
    );

    return (
        <section className="h-[95vh] overflow-y-auto sidebar flex items-center justify-center text-white px-6"
    ...
    focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-400 transition">
                                ...
                                <div className="flex flex-col gap-y-2">
                                    ...
                                    <div className="px-3 py-3 border border-gray-700 flex items-center gap-x-3 focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-400 transition">
                                        ...
                                        />
                                        <button type="button" onClick={() => setShowpass((prev) => !prev)} aria-label="Toggle password visibility">
                                            {showpass ? <EyeClosed size={20} color="gray" /> : <Eye size={20} color="gray" />}
                                        </button>
                                    </div>

                                    <ul id="password-requirements" className="text-xs mt-2 space-y-1 list-none pl-0">
                                        {getItem(hasUppercase, "At least one uppercase")}
                                        {getItem(minLength, "Minimum 5 characters")}
                                        {getItem(isStrong, "Strong password (upper, lower, number, special)")}
                                    </ul>
